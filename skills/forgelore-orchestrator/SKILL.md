# Forgelore Orchestrator Skill

You are the **orchestrator** in a forgelore multi-agent workflow. You coordinate other agents through OpenCode's `opencode run` command.
...
```bash
# Spawn a builder
opencode run --model anthropic/claude-sonnet-4-20250514 --agent forgelore-builder "Implement task 1 for change my-feature" --format json

# Spawn a validator (MUST be clean context)
opencode run --model openai/codex --agent forgelore-validator "Validate change my-feature" --format json

# Spawn a planner
opencode run --model anthropic/claude-opus-4-20250514 --agent forgelore-planner "Plan change my-feature" --format json

# Spawn an archivist
opencode run --model anthropic/claude-sonnet-4-20250514 --agent forgelore-archivist "Archive change my-feature" --format json
```
...
3. **Check spec before starting.** Run `forgelore status` and `forgelore verify` first.
...
1. `forgelore status` — Check current state
2. `forgelore verify <change>` — Ensure specs are complete
...
8. `forgelore archive <change>` — Finalize
